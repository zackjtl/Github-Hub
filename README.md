...
GitHub 上的 repo 通常只是代碼的「源頭」 — 實際的網站、文件、設計稿、看板、雲端服務 console 都散落在各個平台。GitHub-Hub 讓你可以為**每個 repo** 釘選任意數量的連結，並依照平台自動套用 icon 與顏色（Vercel、Netlify、Figma、Notion、Linear、Supabase、Firebase、Google Drive…）。
...
這些連結會顯示在 repo 詳細頁的最上方，一鍵直達相關平台。

### 截圖

| Repo 詳細頁 | 筆記 |
|---|---|
| ![Repo 詳細頁](https://raw.githubusercontent.com/zackjtl/Github-Hub/main/Docs/repo-detail.png) | ![Markdown 筆記](https://raw.githubusercontent.com/zackjtl/Github-Hub/main/Docs/notes.png) |
...
**Repo 頁面 README 的圖片建議怎麼寫？**
建議使用**絕對圖片網址**（例如 `https://raw.githubusercontent.com/...` 或其他可公開存取的完整 URL）。相對路徑在不同部署環境下可能無法正確顯示。
