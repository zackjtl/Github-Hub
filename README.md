...
用 "Use this template" 建立 repo 時，GitHub 會立刻推一筆 commit，這時候 Pages 還沒啟用，所以**第一次部署幾乎必定會失敗**（紅色 ❌，錯誤訊息類似 `Failed to create deployment (status: 404)`）。這是正常的。修復方式：

1. 進 **Actions** 分頁
2. 點開最上面那次失敗的 **Deploy GitHub Dashboard to Pages**
3. 右上角點 **Re-run all jobs** → **Re-run jobs**

這次會成功（綠色 ✅），約 1～2 分鐘。

之後每次推到 `main` 都會自動部署，不需要再手動操作。網站會出現在：
