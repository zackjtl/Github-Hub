...
這個 workflow 預設只支援手動觸發，所以用 Fork 或 "Use this template" 建立 repo 後，不會立刻自動執行。

請先去 **Settings → Pages** 把 Source 改成 **GitHub Actions**，然後回到 **Actions** 頁面點擊 **Run workflow**。

執行成功（綠色 ✅）後，約 1～2 分鐘可在下列網址看到網站：
