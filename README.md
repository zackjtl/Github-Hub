...
> ⚠️ 如果你是用 "Use this template" 建立 repo，第一次部署可能在你開啟 Pages 之前就已經自動觸發並失敗（錯誤訊息會是 `Failed to create deployment (status: 404) ... Ensure GitHub Pages has been enabled`）。完成上面 Pages 設定後，到 **Actions** 分頁點失敗那次 → **Re-run all jobs** 重跑就會成功。

### 3. 觸發部署

repo 內附的 GitHub Actions workflow（`.github/workflows/deploy-pages.yml`）會在每次推到 `main` 時自動建置部署。

- 如果你是用 "Use this template" 建的，第一次推送已經發生了 — 到 **Actions** 分頁就能看到結果（記得先做完上一步再 Re-run）。
